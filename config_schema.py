...
`user_config.json` in the per-user data folder (outside the project), which the config/*.py modules load and
...
SCHEMA = [
    {
        ...
        "fields": [
            ...
            _f("Account", "secrets", "password", "LinkedIn password", "password",
               "Your LinkedIn password. Never uploaded anywhere, and kept only while this tool is open - it is erased when you close the tool (unless you save it in a backup file). Optional - leave blank to log in manually."),
            _f("Account", "secrets", "google_email", "Google account for company sites", "text",
               "Many company job sites offer 'Sign in with Google'. Enter the Google account to use there. If your browser is already signed in to Google, the tool just picks this account on Google's screen. It never types a Google password: if Google asks for one (or 2-step verification), the tool waits for you to finish it in the browser window."),
    ...
    },
    {
        ...
        "fields": [
            ...
            _f("Profile", "personals", "email", "Email for applications", "text",
               "The email address to put on job applications. If left blank, your Google account email (Account tab) or your LinkedIn login email is used."),
            ...
            _f("Profile", "questions", "years_of_experience", "Years of experience", "text",
               "What to answer for 'how many years of experience do you have' questions. A number in text, e.g. 0, 1, 3, 5. Leave it blank if you'd rather not say: the tool then won't type a number for you."),
            ...
            _f("Profile", "questions", "us_citizenship", "Citizenship status", "select",
               "Your work-authorization / citizenship status for US applications. Leave it unanswered if you're not sure - the tool then won't state one.",
            ...
            _f("Profile", "questions", "desired_salary", "Desired salary", "number",
               "Your expected salary or CTC as a plain number (no currency symbols or commas), e.g. 100000. Some forms only accept numbers. 0 means 'not set': the tool then leaves salary questions for you instead of typing a number."),
            _f("Profile", "questions", "current_ctc", "Current salary", "number",
               "Your current salary or CTC as a plain number, e.g. 80000. 0 means 'not set': the tool then leaves the question for you."),
            _f("Profile", "questions", "notice_period", "Notice period (days)", "number",
               "Your notice period in days, e.g. 0, 15, 30 (0 = you can start immediately). The tool converts to weeks or months if a form asks that way. -1 means 'not set': the tool then leaves the question for you."),
            ...
            _f("Profile", "questions", "default_resume_path", "Default resume path", "text",
               "Path to the resume file to upload. Give a full path (e.g. C:\\Users\\you\\Documents\\resume.pdf), or a relative one like all resumes/default/resume.pdf, which is looked up in the tool's private data folder (%LOCALAPPDATA%\\AutoJobApplier) - never inside the project folder, so sharing the project never shares your resume. If the file is missing, the tool keeps your last resume on LinkedIn."),
            ...
            _f("Profile", "personals", "country", "Country", "text",
               "The country you live in. Used to answer 'are you authorized to work in <country>?' questions: the tool only says yes for this country, and leaves questions about any other country for you."),
    ...
    },
    {
        ...
        "fields": [
            ...
            _f("Search", "search", "easy_apply_only", "Easy Apply only", "bool",
               "On = only LinkedIn Easy Apply jobs. Off = every kind of job, including ones that apply on the company's own website (the tool opens the site, signs in with Google if asked, and fills the form from your resume and details)."),
    ...
    },
    {
        ...
        "fields": [
            ...
            _f("Run settings", "search", "pause_after_filters", "Pause after search filters", "bool",
               "Pause once per search term after LinkedIn's filters are applied, so you can double check the results before it starts applying. Turn off for fully unattended runs - this is the single biggest source of 'stuck waiting for a click' if you have many search terms, since it prompts once per term."),
            _f("Run settings", "settings", "external_apply_enabled", "Apply on company websites", "bool",
               "For jobs that don't use Easy Apply, open the company's application page and fill it in from your resume and details. Needs 'Easy Apply only' to be off in the Search tab. If this is off, only the application link is saved. The tool never submits a form with a required question empty, and never guesses an answer it doesn't know."),
            _f("Run settings", "settings", "use_google_login", "Use Google sign-in on company sites", "bool",
               "When a company site asks you to sign in or create an account, click its 'Sign in / Sign up with Google' button."),
            _f("Run settings", "settings", "external_manual_wait_seconds", "Wait for me on hard steps (seconds)", "number",
               "If a company site needs a person - a CAPTCHA, Google's password or 2-step screen, a question the tool can't answer - wait this many seconds for you to handle it in the browser window before giving up on that job. 0 = don't wait. Always 0 when 'Run in background' is on."),
            _f("Run settings", "settings", "max_applications_per_day", "Max applications per day", "number",
               "Safety cap on total applications (Easy Apply + external) across all your runs today. Protects your LinkedIn account from being flagged for automated activity. LinkedIn itself caps Easy Apply at roughly 50/day platform-wide (even for Premium) - keep this below that so the tool stops itself gracefully instead of repeatedly hitting LinkedIn's own wall. -1 disables the cap (not recommended)."),
            # --- advanced run options ---
            _f("Run settings", "settings", "external_apply_timeout_seconds", "Time limit per company site (seconds)", "number",
               "Give up on a single company website after this long, so one stuck site never holds up the whole run. At least 30.", advanced=True),
            _f("Run settings", "settings", "external_apply_max_steps", "Page-step limit per company site", "number",
               "Give up on a company website after this many pages/steps. At least 3.", advanced=True),
